document.addEventListener('DOMContentLoaded', () => {
  ...
  // Инициализация основного текста MERROR
  ...
  // Обновление активного постера для мобильной версии
  ...
  function handlePosterNavigation() {
    ...
    // Удаление существующих слушателей событий
    ...
    newPrevBtn?.addEventListener('click', () => {
      currentPosterIndex = (currentPosterIndex - 1 + posterWrappers.length) % posterWrappers.length;
      updateActivePoster();
    });
  ...
  // Массив масок и переменные состояния
  ...
  // Инициализация игры с зеркалами
  ...
  function checkAllInputsFilled() {
    const allFilled = Array.from(posterInputs).every(input => input.value.trim() !== '');
    document.querySelectorAll('.face5-overlay, .face3-overlay, .faceposter3-overlay').forEach(face => {
      face.style.animation = allFilled ? 'pulse 1s infinite ease-in-out' : 'none';
    });
  }
  ...
  changerColor.addEventListener('click', () => {
    ...
    maskImage.addEventListener('animationend', () => {
  ...
  function showPage(pageNumber) {
    ...
    if (posterPageElement) posterPageElement.style.display = pageNumber === 3 ? 'block' : 'none';
    if (drawingPageElement) drawingPageElement.style.display = pageNumber === 4 ? 'block' : 'none';
  ...
  function startCursorAnimation() {
    ...
    cursorInterval = setInterval(() => {
      ...
      document.body.style.cursor = `url(./images/cursor${cursorBlue ? 'Blue' : 'Black'}.svg), auto`;
    }, 100);
  }
